...
# --- FUNCIÓN DE BÚSQUEDA ESPECIAL TRADE REPUBLIC ---
def get_tr_data(ticker_raw):
    # Diccionario de traducción para los tickers más usados en TR
    ...
    try:
        ...
        info = t.info
        # Si no hay precio en el ticker .LS, probamos el original
...
if ticker_input:
    ...
    if accion and info_main:
        ...
        # Extraemos precios (Directamente en EUR si es .LS)
        ...
        m3.success(f"⏱️ Pulso: 5s (Sincronizado L&S)")
        st.markdown("---")
        ...
        h1, h2 = st.columns(2)
        h1.warning(f"🕒 **PICO MÁXIMO:** Se estima **{t_max:.2f}** a las **15:30**")
        h2.info(f"🕒 **SUELO MÍNIMO:** Se estima **{s_min:.2f}** a las **10:15**")
        ...
        if vol_actual > (vol_media * 0.5):
            ...
            f2.write("✅ Grandes fondos detectados en el mercado europeo.")
        else:
            f1.warning("👥 **FLUJO:** Sentimiento Minorista")
            f2.write("⚠️ Volumen estándar de pequeños inversores.")
        ...
        st.markdown(f"""
            <div style='background-color:{color_rec}; padding:20px; border-radius:10px; text-align:center;'>
                ...
                <h3 style='color:white; margin-top:10px;'>Precio Objetivo Analistas: {target:.2f} {currency}</h3>
            </div>
