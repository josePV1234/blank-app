...
# --- FUNCIÓN DE BÚSQUEDA ROBUSTA (Para que SAP funcione) ---
...
if ticker_input:
    ...
    if accion and info_main:
        ...
        # Asumimos abierto si es horario comercial europeo para L&S
        ...
        # Simulamos profundidad de mercado como en tu código original
        ...
        # --- SECCIÓN 3: RANGOS ---
        st.markdown("---")
        t_max = info_main.get('dayHigh', p_real)
        s_min = info_main.get('dayLow', p_real)
        r1, r2 = st.columns(2)
        ...
        p2.markdown(f"<div style='background-color:#0e1117; padding:15px; border-left:5px solid #ffaa00; border-radius:5px;'><h3 style='color:#ffaa00; margin:0;'>MÍNIMO Previsto:</h3><h1 style='color:#s_min - diff:.2f} {currency}</h1></div>", unsafe_allow_html=True)
